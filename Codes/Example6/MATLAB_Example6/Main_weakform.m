...
V = [0.0 0.0; 0.5 0; 0.5 0.5; 1.0 0.5; 1.0 1.0;0.0 1.0];
R1 = [3,size(V,1),V(:,1)',V(:,2)']';
model = createpde();
geometryFromEdges(model, decsg(R1));  % Use decsg function
mesh = generateMesh(model,Hmax=0.0075,Hgrad=2.0,GeometricOrder='linear'); 
nodes = mesh.Nodes';
...
%% Saving
save('Package_L_Shaped_Weak_Form','nodes','phi','dphi_dx','dphi_dy');
